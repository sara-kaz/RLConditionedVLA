...
Figure~\ref{fig:cosine} (alignment cosines) and Figure~\ref{fig:curves}
(validation curves) together provide complementary evidence.
The rising cosines for Full \modelname\ confirm that the InfoNCE loss
progressively anchors successful embodied consequence embeddings toward the
instruction, while flat cosines for the BC baseline confirm that the alignment
signal is absent without the loss.
The training curves show that the $+0.65$\,pp advantage of Full \modelname\
over BC is consistent across seeds and epochs, ruling out seed-specific luck.
