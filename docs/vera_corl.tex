...
and re-encoded as an embodied consequence token \Lexp, capturing
...
The full six-variant ablation study spanning both Language-Table and
...
The \textbf{Embodied Consequence Token} \Lexp\ (Stream~3b) jointly
...
\modelname\ introduces a five-stream closed-loop architecture that fuses vision, instruction, action narration, embodied consequence, and numerical history through a LLaMA-style bidirectional Transformer with ViLT modality-type embeddings (\S\ref{sec:model}).
The \texttt{verbalize\_consequence()} function (\S\ref{sec:experience}) maps scalar reward and spatial outcome to up to 16 semantically distinct CLIP-compatible strings; a reward-weighted InfoNCE loss (\S\ref{sec:alignment}) with exponential weighting $\exp(5\tilde{r})$ then aligns successful embodied knowledge embeddings toward the task instruction.
We additionally identify and correct two latent design errors in naive closed-loop VLA implementations---a causal mask that blinds feedback tokens to the visual scene and a CLS placement that severs aggregation (\S\ref{sec:bug})---and introduce an expand-compress CoT-lite action head with $K{=}4$ chunking that yields $4{\times}$ the supervised signal per step.
...
attempted action and its embodied consequence---a fundamentally different role.

\paragraph{Action chunking and fusion architecture.}
$\pi_0$~\citep{black2024pi0} and GR-1~\citep{wu2023gr1} predict a window of $K$ future actions
...
consequence and action narration tokens, so temporal consistency is informed by outcome feedback.
...
\begin{enumerate}[leftmargin=1.5em,itemsep=1pt]
  ...
  \item \textbf{Embodied Consequence} (\Lexp): scalar $r_{t-1}$ and
...
\begin{figure}[t]
  ...
  \textbf{(3a) Action Narration}: the previous action verbalized from a fixed vocabulary (e.g.\ \textit{``I pushed the block left''}) and re-encoded; a learned reward gate scales the token by step quality.
  \textbf{(3b) Embodied Knowledge}: the observed outcome verbalized as a consequence string (e.g.\ \textit{``block moved closer to goal''}) and re-encoded---capturing \emph{what happened}, orthogonal to \emph{what was attempted}.
  ...
  The dashed feedback arrow shows that action narration, embodied knowledge, and history re-enter as inputs at $t{+}1$, closing the loop.}
  \label{fig:arch}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Language-Table ablation results (all six conditions, 3 seeds each).
    Individual seed values are overlaid as scatter points.
    Full \modelname\ achieves $+0.65$\,pp over the BC baseline across all seeds;
    every partial variant falls between BC and the full model,
    confirming complementarity of the two language streams.}
...
The complete six-variant ablation study—spanning both Language-Table and
CALVIN D$\to$D—is in progress.
Table~\ref{tab:ablations} shows the planned structure for the combined results.

\begin{table}[h]
\centering
\caption{Six-variant ablation study across both environments (3 seeds each, mean$\pm$std).
...
without the loss.
Validation accuracy learning curves show that the $+0.65$\,pp advantage
...
In representative Language-Table rollouts, \Lexp\ triggers a directional correction
...
CLS-to-token attention weights further show that after a failed step \modelname\
