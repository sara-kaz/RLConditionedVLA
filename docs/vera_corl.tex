...
\begin{table}[h]
\centering
\caption{Prior work on Language-Table and CALVIN D$\to$D.
  \modelname\ reports action-classification accuracy (our metric, currently);
  all other entries report simulation task success.
  $^\star$D$\to$D: train on environment D, evaluate on environment D (same domain).
  $^\dagger$ABCD$\to$D: train on A,B,C,D; evaluate on D (unseen visual domain, harder).
  Chain = average consecutive tasks completed (out of 5).}
...
\begin{tabular}{llccc}
\toprule
\textbf{Method} & \textbf{Benchmark} & \textbf{Success / Acc} & \textbf{Chain} & \textbf{Metric} \\
\midrule
\multicolumn{5}{l}{\textit{Language-Table (Lynch et al., 2023) — task success in simulation}} \\
BC baseline~\citep{lynch2023languagetable}
  & Lang-Table (sim)   & ${\sim}$40\%  & — & Task success \\
Language-Table model~\citep{lynch2023languagetable}
  & Lang-Table (sim)   & ${\sim}$68\%  & — & Task success \\
IAFM~\citep{durante2024agent}
  & Lang-Table (sim)   & ${\sim}$65\%  & — & Task success \\
\textbf{\modelname\ Mode B (ours)}
  & Lang-Table (val)   & $\mathbf{51.4\pm1.9\%}$ & — & \textbf{Cls. acc.}$^\ddagger$ \\
\midrule
\multicolumn{5}{l}{\textit{CALVIN (Mees et al., 2022) — 1-task success + chain (higher is better)}} \\
MCIL~\citep{mees2022calvin}
  & CALVIN D$\to$D$^\star$     & 52.8\%   & 0.67 & Task success \\
HULC~\citep{mees2022calvin}
  & CALVIN D$\to$D$^\star$     & 91.8\%   & 3.06 & Task success \\
MCIL~\citep{mees2022calvin}
  & CALVIN ABCD$\to$D$^\dagger$ & 35.0\%  & 0.53 & Task success \\
HULC~\citep{mees2022calvin}
  & CALVIN ABCD$\to$D$^\dagger$ & 88.9\%  & 2.82 & Task success \\
HULC++~\citep{mees2022calvin}
  & CALVIN ABCD$\to$D$^\dagger$ & ${\sim}$90\% & 3.07 & Task success \\
GR-1~\citep{wu2024gr1}
  & CALVIN ABCD$\to$D$^\dagger$ & 85.4\%  & 4.21 & Task success \\
\textbf{\modelname\ Mode B (ours)}
  & CALVIN D$\to$D (planned)   & \textit{pending} & — & \textbf{Cls. acc.}$^\ddagger$ \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^\ddagger$Cls.\ acc.\ = discrete 8-bin action-classification accuracy; not directly
comparable to task success.} \\
\multicolumn{5}{l}{\footnotesize Simulation rollout results on both benchmarks are planned for the camera-ready version.} \\
\end{tabular}
\end{table}

\paragraph{Context for our classification accuracy.}
Our 51.4\% action-classification accuracy on Language-Table measures how
well the policy's predicted action bin matches the expert's 8-bin discretised
movement. Lynch et al.'s BC baseline reaches ${\sim}$40\% task success in
full simulation; ours reaches 50.8\% classification accuracy on held-out
windows---consistent with being a strong behaviour-cloned policy before
closed-loop evaluation.
The key comparison in Table~\ref{tab:lt_results} is therefore
\emph{relative}: \modelname\ consistently outperforms the BC baseline across
all three seeds, confirming the contribution of the dual language-alignment
streams independent of the absolute metric scale.
...
Table~\ref{tab:lt_results} contains one pending row (no-language-feedback on
Language-Table); Table~\ref{tab:ablations} reports one completed column
(Language-Table: Full VERA and BC) with all CALVIN D$\to$D entries and
remaining ablations in progress.
