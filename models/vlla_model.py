"""
VLLA: Vision-Language-Language-Action Model
============================================
Four input streams, two feedback channels back to the policy:

  Stream 1 — Vision                    : frozen CLIP ViT-B/32 image encoder
  Stream 2 — Language (instruction)    : frozen CLIP text encoder
  Stream 3 — Language (action feedback): previous action verbalized, re-encoded via
                                         the SAME frozen CLIP text encoder        [NOVEL]
  Stream 4 — Action-Reward History     : numerical (action_idx, reward) pairs

Two feedback lines:
  Feedback A (Semantic):  prev_action → vocabulary → CLIP text → reward gate → 256-dim token
  Feedback B (Numerical): (action_idx, reward) × H → TemporalHistoryTransformer → H tokens

Fusion backbone — LLaMA + ViLT hybrid:
  ┌─ ViLT modality-type embeddings (Kim et al., 2021)
  │   Learnable type vectors distinguish: INSTRUCTION | ACTION_LANG | VISION | HISTORY
  │   Added to each token BEFORE the main fusion stack.
  │
  └─ LLaMA-style decoder blocks (Touvron et al., 2023)
      ...
      • Causal mask   (CLS at last position attends to all prior tokens)

Token sequence:
  [ L_instr | L_action | L_consequence | V_1 | ... | V_T | H_1 | ... | H_H | CLS ]

  L_action      — what the agent did  (pre-tokenized vocab lookup → frozen CLIP)
  L_consequence — what happened next  (dynamically verbalized from reward + state_delta → frozen CLIP)

Ablation flags:
  vlla.use_lang_feedback      → False : base VLA without Stream 3
  vlla.use_temporal_history   → False : flat positional history
  vlla.use_reward_gate        → False : no reward-gated semantic feedback
  vlla.use_consequence_token  → False : no consequence language token (Stream 3b)
  vlla.alignment_loss_coef    → 0     : no contrastive auxiliary loss
"""
...
class ViLTModalityEmbedding(nn.Module):
    ...
    We extend this to five modality types for VLLA:
...
# ═════════════════════════════════════════════════════════════════════════════
# Part 3: Existing VLLA Components
# ═════════════════════════════════════════════════════════════════════════════
...
def verbalize_consequence(reward: float, delta_dist: Optional[float] = None) -> str:
    """
    Generate a natural-language description of the CONSEQUENCE of an action.

    Encodes what happened as a result of the action — not what the action was.
    This is the key distinction: the action token says "I moved forward",
    while the consequence token says "As a result, I got closer to the goal".

    Args:
        reward    : scalar reward signal received after the action.
        delta_dist: signed change in distance to goal (negative = got closer).
                    When None, falls back to reward-only description.

    Returns a short natural-language string suitable for CLIP encoding.
    """
    if delta_dist is not None:
        if delta_dist < -0.05:
            return "As a result, I got closer to the goal"
        elif delta_dist > 0.05:
            return "As a result, I moved farther from the goal"
        else:
            return "As a result, my position relative to the goal barely changed"
    # Fallback: reward-only verbalization
    if reward > 0.5:
        return "The action was successful and I received a positive reward"
    elif reward > 0.0:
        return "The action was partially successful with a small positive reward"
    elif reward < -0.05:
        return "The action failed and I received a penalty"
    return "The action had no clear effect on my progress"
...
class ActionRewardHistoryEncoder(nn.Module):
    """
    Feedback Channel B (Numerical):
      action_embed(action_idx) ⊕ reward_proj(reward) → fusion → sinusoidal PE
      → optional TemporalHistoryTransformer → (B, H, d_model)
    """

    def __init__(self, num_actions: int, history_len: int, d_model: int = 256,
                 dropout: float = 0.1, use_temporal_transformer: bool = True):
        super().__init__()
        ...
        self.fusion       = nn.Sequential(
            nn.Linear(d_model * 2, d_model, bias=False), RMSNorm(d_model), nn.SiLU()
        )
    ...
    def forward(self, action_hist: torch.Tensor, reward_hist: torch.Tensor) -> torch.Tensor:
        a_emb  = self.action_embed(action_hist)
        r_emb  = self.reward_proj(reward_hist.unsqueeze(-1).float())
        tokens = self.fusion(torch.cat([a_emb, r_emb], dim=-1))
...
# ═════════════════════════════════════════════════════════════════════════════
# Part 4: Main VLLA Model
# ═════════════════════════════════════════════════════════════════════════════

class VLLAModel(nn.Module):
    """
    Vision-Language-Language-Action (VLLA) Model.
    ...
    forward() returns a dict (not a bare tensor):
      ...
      alignment_score : (B,) cosine similarity
    ...
    def forward(
        ...
        state_delta:     Optional[torch.Tensor] = None,  # (B,) signed dist-to-goal change [NEW]
    ) -> Dict[str, Optional[torch.Tensor]]:

        B = frames.size(0)

        # 1. Encode all streams
        vis_tokens,  _         = self.encode_frames(frames), None
        ...
        return {
            ...
            "alignment_score":    alignment_score,    # cosine(instr, action_lang)
            "consequence_score":  consequence_score,  # cosine(instr, consequence) [NEW]
            "instr_emb":          instr_emb,          # (B, 512) raw CLIP
            "action_lang_emb":    action_lang_emb,    # (B, 512) raw CLIP
            "consequence_emb":    consequence_emb,    # (B, 512) raw CLIP [NEW]
        }
    ...
    def predict(self, frames, lang_tokens, action_hist, reward_hist,
                prev_action_idx=None, prev_reward=None) -> int:
        self.eval()
        with torch.no_grad():
