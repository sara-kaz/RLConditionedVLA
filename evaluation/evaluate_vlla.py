...
Usage
-----
  python -m evaluation.evaluate_vlla \\
...
Ablation comparison (runs all ablations back-to-back):
  python -m evaluation.evaluate_vlla \\
      --config configs/config.yaml \\
      --checkpoint checkpoints/best_sft_vlla.pt \\
      --ablation-table
"""
...
def build_vlla_from_cfg(cfg: dict, device: str) -> VLLAModel:
...
def load_checkpoint(model: VLLAModel, checkpoint_path: str, device: str):
...
def evaluate_multi_seed(
    ...
    model  = build_vlla_from_cfg(cfg, device)
...
def run_ablation_table(
    ...
    for name, overrides in ABLATION_PRESETS.items():
        ...
        for k, v in overrides.items():
            ablation_cfg["vlla"][k] = v

        model = build_vlla_from_cfg(ablation_cfg, device)
...
def main():
    ...
    if args.ablation_table:
        ...
        out_path = out_dir / "vlla_eval_results.json"
        with open(out_path, "w") as f:
            json.dump(results, f, indent=2)
        print(f"\nSaved to {out_path}")
